 ...
 */
async function seedRandomVillages(count: number | null = null) {
    ...
    try {
        ...
        const villageValues: string[] = [];
        const villageParams: (string | number)[] = [];
        let paramIndex = 1;
        const villageMap: { tile_id: number; chunk_index: number }[] = [];
        ...
        if (villageValues.length > 0) {
            ...
            if (insertedVillages.length > 0) {
                ...
                try {
                    ...
                    console.warn('[villageSeeder] could not batch update tiles_lands.village_id:', (e as Error).message);
                }
            }
        }
...
async function buildResidencyUpdates(tileIds) {
    ...
    // Build batch updates
    const residencyUpdates: { residency: number; personIds: number[] }[] = [];
    const housingUpdates: { villageId: number; slots: number[] }[] = [];
    ...
    try {
        ...
        for (const r of available) {
            ...
            try {
                ...
                console.warn('[villageSeeder] could not update tiles_lands.village_id - skipping:', (e as Error).message);
            }
        }
