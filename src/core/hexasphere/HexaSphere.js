...
const Face_1 = tslib_1.__importDefault(require("./Face"));
...
class Hexasphere {
    constructor(radius, numDivisions, hexSize) {
        ...
        for (let f = 0; f < faces.length; f++) {
            ...
            const left = face.points[0].subdivide(face.points[1], numDivisions, getPointIfExists);
            const right = face.points[0].subdivide(face.points[2], numDivisions, getPointIfExists);
        ...
        for (const p in points) {
            const newTile = new Tile_1.default(points[p], hexSize);
        ...
        for (let t = 0; t < this.tiles.length; t++) {
            ...
            tile.neighbors = tile.neighborIds.map((item) => this.tileLookup[item]);
        }
...
//# sourceMappingURL=HexaSphere.js.map