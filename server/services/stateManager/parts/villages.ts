...
async function insertPendingVillages(villageData, PopulationState) {
    ...
    const villageIdMappings: { tempId: number; newId: number }[] = [];
    ...
    if (pendingVillageIds.length > 0) {
        ...
        for (const tempId of pendingVillageIds) {
            try {
                ...
                console.warn('[stateManager] Failed to insert pending village:', (err as Error).message || err);
            }
        }
