...
import { Pool } from 'pg';
import { FamilyData, PersonData, CalendarDate } from '../../../types/global';
 ...
 */
async function createFamily(pool: Pool | null, husbandId: number, wifeId: number, tileId: number): Promise<FamilyRecord | null> {
    ...
    try {
        ...
        } catch (e: unknown) { const err = e as Error; console.warn('[createFamily] removeEligiblePerson failed for husband:', err?.message ?? e); }
        try {
            await PopulationState.removeEligiblePerson(wifeId, tileId, 'female');
        } catch (e: unknown) { const err = e as Error; console.warn('[createFamily] removeEligiblePerson failed for wife:', err?.message ?? e); }
        ...
        if (lockToken) {
            try { await releaseLock(lockKey, lockToken); } catch (e: unknown) { const err = e as Error; console.warn('[createFamily] failed to release lock:', err?.message ?? e); }
        }
    }
}
 ...
 */
async function startPregnancy(pool: Pool | null, calendarService: CalendarService | null, familyId: number): Promise<FamilyRecord | null> {
    const PopulationState = deps.getPopulationState();
    const { acquireLock, releaseLock } = deps.getLock();
    ...
    try {
        ...
        console.error(`Error starting pregnancy for family ${familyId}:`, err?.message ?? error);
        throw error;
    } finally {
        ...
        if (lockToken) {
            try {
                ...
                console.warn(`[startPregnancy] Failed releasing lock for family ${familyId}:`, err?.message ?? e);
            }
        }
    }
}
 ...
 */
async function deliverBaby(pool: Pool | null, calendarService: CalendarService | null, populationServiceInstance: PopulationServiceInstance | null, familyId: number): Promise<DeliveryResult | null> {
    const { acquireLock, releaseLock } = deps.getLock();
    const lockKey = `lock:family:${familyId}`;
    let lockToken: string | null = null;
    ...
    try {
        ...
        if (family.husband_id) {
            const fatherFromRedis = await PopulationState.getPerson(family.husband_id) as PersonRecord | null;
            if (fatherFromRedis) {
        ...
        const updatedChildrenIds: number[] = [...(family.children_ids || []), babyId];
        ...
        if (lockToken) {
            try { await releaseLock(lockKey, lockToken); } catch (e: unknown) { const err = e as Error; console.warn('[deliverBaby] failed to release lock:', err?.message ?? e); }
        }
    }
}
 ...
 */
async function getFamiliesOnTile(pool: Pool | null, tileId: number): Promise<FamilyRecord[]> {
    try {
        const PopulationState = deps.getPopulationState();
        const allFamilies = await PopulationState.getAllFamilies() as FamilyRecord[];
        return allFamilies.filter((f: FamilyRecord) => f.tile_id === tileId);
    } catch (error: unknown) {
 ...
 */
async function processDeliveries(pool: Pool | null, calendarService: CalendarService | null, populationServiceInstance: PopulationServiceInstance | null, daysAdvanced: number = 1): Promise<number> {
    try {
        const PopulationState = deps.getPopulationState();
        ...
        if (storage.isAvailable() && typeof storage.atomicPopDueFertile === 'function') {
            ...
            const allFamilies = await PopulationState.getAllFamilies() as FamilyRecord[];
            readyFamilies = allFamilies.filter((f: FamilyRecord) => {
            ...
            try {
                ...
                try {
                    if (res && res.family) {
                        ...
                        const wife = await PopulationState.getPerson(f.wife_id) as PersonRecord | null;
                        if (wife && wife.date_of_birth) {
                            // get current date
                ...
                if (lockToken) {
                    try { await releaseLock(lockKey, lockToken); } catch (e: unknown) { const err = e as Error; console.warn('[processDeliveries] failed to release lock:', err?.message ?? e); }
                }
            }
        }
 ...
 */
async function getFamilyStats(pool: Pool | null): Promise<FamilyStats> {
    try {
        const PopulationState = deps.getPopulationState();
        const allFamilies = await PopulationState.getAllFamilies() as FamilyRecord[];

        const totalFamilies = allFamilies.length;
        const pregnantFamilies = allFamilies.filter((f: FamilyRecord) => f.pregnancy).length;
        const totalChildren = allFamilies.reduce((sum: number, f: FamilyRecord) => sum + (f.children_ids?.length || 0), 0);
 ...
 */
async function formNewFamilies(pool: Pool | null, calendarService: CalendarService | null): Promise<number> {
    try {
        const PopulationState = deps.getPopulationState();
        ...
        for (const tileId of tileSet) {
            try {
                ...
                console.warn('[formNewFamilies] Error processing tile:', tileId, error?.message ?? err);
            }
        }
